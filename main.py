...
def window(name,no,state,num):  #  GUI
    ...
    label_0 = Label(root, text="ANPR", width=20, font=("bold", 35), bg="yellow", fg="red")
...
cap = cv2.VideoCapture(1, cv2.CAP_DSHOW)  # Use index 1, confirmed working

# ✅ Check if opened successfully
if not cap.isOpened():
    print("❌ Error: Could not open DroidCam feed. Ensure DroidCam is running and connected.")
    exit()
...
while True:
    ...
    # Load cascade once (can be moved outside loop for optimization)
